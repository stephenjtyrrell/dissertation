...
.PHONY: help tf-init tf-fmt tf-validate tf-plan tf-apply tf-destroy policy-tf policy-k8s k8s-dry-run argocd-apply argocd-status argocd-test clean all
...
k8s-dry-run: ## Validate Kubernetes manifests with kubectl client-side dry run
	kubectl create --dry-run=client --validate=false -f k8s/app
